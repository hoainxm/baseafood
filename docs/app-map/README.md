...
| [37-doi-soat-hddt.md](37-doi-soat-hddt.md) | Đối soát hóa đơn điện tử (cổng thuế) ⇄ phần mềm kế toán từ 1 file Excel: khóa MST·ký hiệu·số HĐ, quy VND, ngưỡng chỉnh được, **xuất Excel GIỮ NGUYÊN định dạng file vào + bám bố cục file mẫu kế toán** (sửa tại chỗ bằng `exceljs`: cột KẾT QUẢ ở đầu, khối phân tích ở cuối, 3 sheet tổng hợp lên đầu; chèn cột thì phải dịch tham chiếu công thức); nhận cả `.xls` và sổ gộp nhiều tháng; biết hóa đơn "đã bị thay thế" thì KHÔNG cần vào sổ; **so hai bản HĐĐT** (thuế gửi ⇄ tự tải) và **lọc hóa đơn cùng MST cùng ngày**; công cụ kế toán không đọc DB | `src/features/doi-soat/**`, `src/lib/doiSoat*.ts` |
