...
<!-- updated: 2026-09-18 — truy xuất lô QR: `src/lib/truyXuatLo.ts` (hàm THUẦN: đọc mã QR 3 dạng, nhãn lô, tìm lô, truy ngược/xuôi, cân bằng kg, danh sách lô để chọn) · `src/features/shared/` thêm `GanLoDauVao.tsx` (hộp gắn lô đầu vào), `KhungQuetQr.tsx` (khung camera dùng chung, id riêng mỗi khung), `useDuLieuTruyXuat.ts` (gom 10 bảng), `QrTemLoIn.tsx` thêm `TemLoQr` (tem cho mọi loại lô). Spec: `docs/spec/qr-truy-xuat-lo.md`. Không dep mới. -->
