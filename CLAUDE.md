...
| [`spec/qr-truy-xuat-lo`](docs/spec/qr-truy-xuat-lo.md) | **Truy xuất theo lô bằng QR** (`/qr` hộ chiếu lô · gắn lô ở `/wip` + `/packaging`): chuẩn GS1/EPCIS/GDST/FSMA 204/TT 02-2024, mô hình 3 loại lô + bảng `lot_inputs`, lộ trình 3 đợt, câu còn treo |
...
- **Chỗ đứt gãy chuỗi** (chi tiết [`flow §3`](docs/trien-khai/flow-end-to-end-2-bo-phan.md)): (G1) ✅ **KHÉP VÒNG + HẾT CỘNG ĐÔI (hướng 1, chốt 2026-09-06)**: chốt ngày SX ghi "còn dở" **tách theo loại NL** (`production_locks.leftover_by_material`, mig `0038`), sổ `/nxt-nl` lấy làm **đông gửi CHÍNH**: `dongGui = conDoSX>0 ? conDoSX : Chuyển-kỳ-âm` (không cộng cả hai) → tồn cuối = tồn đầu + đông gửi − xả đông. Kế toán khỏi khai Chuyển kỳ âm; dữ liệu cũ (carry âm) không hồi quy. /nxt-nl gộp 1 cột "Đông gửi" (nhãn "(SX)"); (G2) ✅ hết chồng: `cold-storage` hút tồn thật (`tinhTon`) + nhãn nhiệt độ minh hoạ, `warehouse` canonical — còn: cảm biến thật; (G3) ✅ đóng gói BTP→TP đã build: màn `/packaging` (phiếu đóng gói BTP tiêu hao→TP ra + hao hụt, `0026`) + tồn TP suy (`tinhTonTP`); bán chọn nguồn Block thô (trừ BTP)/Đóng gói (trừ TP) — còn: tách kho TP theo nhiệt/vị trí; (G4) ✅ bán lẻ trừ tồn: dòng bán chọn **nguồn** (`KHO_BAN_LE` trừ BTP / `KHO_TP` trừ TP đóng gói), trừ suy FIFO qua `tinhTon(...,banLe)`/`tinhTonTP`, nhất quán warehouse/cold-storage/orders/packaging.
...
- **Bổ sung họp 2026-09-02** (chi tiết [`hop-2026-09-02`](docs/trien-khai/hop-2026-09-02-form-nhap-trace-gia-qc.md)) — **ĐÃ BUILD đợt 1 (2026-09-05):** (4) ✅ **mã số KH/đại lý** (tái dùng cột `code`, combobox gõ số ra tên, kiểm trùng — không migration); (3) ✅ **mã lô nội bộ + ô SSCC** ở Nhập hàng (`0035`, cột `import_shipments.lot_code`/`sscc_code`); (5) ✅ **màn QC checklist chấm điểm** `/qc` (`0036`, bảng `qc_checklists`+`qc_locks`; chốt ngày + ghi bù) — **ảnh để sau** (chưa có Storage). **ĐÃ BUILD thêm (2026-09-08):** (1) ✅ **OCR chụp phiếu nhập → tự điền form** — nút "Nhận diện từ ảnh phiếu" ở `/imports`, OCR **client-side Tesseract** (`lib/ocr.ts` nạp động, không key/dịch vụ) tự điền NHÁP ngày·đại lý·dòng loại NL+kg (người soát lại), ảnh phiếu lưu Storage bucket `qc` (`0040` cột `import_shipments.scan_path`, `OcrPhieuNhap.tsx`). Bộ nhận diện tách để nâng cấp cloud (Google Vision) sau. **CÒN CHỜ/ĐỂ SAU:** (2) 🟨 **giá bình quân gia quyền** — **CHỐT PA trước khi đụng `balancingCalc.ts`** (lô cao→SX/thấp→kho VS bình quân gia quyền ngày); (3b) ✅ **truy xuất lô bằng QR đợt 1 (2026-09-18)** — hộ chiếu lô `/qr` truy ngược/xuôi + cân bằng kg, gắn lô NL ở `/wip` và lô BTP ở `/packaging` (quét/gõ/chọn), QR là đường link, mig `0046_lot_inputs` ⚠ chạy trên DB thật trước khi dùng nút Gắn lô ([spec](docs/spec/qr-truy-xuat-lo.md)); còn đợt 2 (QR vị trí kho, FIFO theo lô, bán lẻ gắn lô) + đợt 3 (SSCC/GDST); (1b) OCR nâng độ chính xác chữ tay bằng cloud (chờ duyệt phí + key); form xưởng theo "tờ chị Trúc"; (6) 🅿️ lương/BHXH/nghỉ phép, điện thoại xưởng.
- **Đợt 2 (2026-09-05):** ✅ **PWA + mobile-first + tắt zoom** (`public/manifest.webmanifest`+`public/sw.js` network-first chỉ ở PROD + icons + viewport `user-scalable=no` + `touch-action:manipulation`) — cài được như app, khóa pinch/double-tap; ✅ **NR-5 mã số mặt hàng** (kiểm trùng + combobox prefix "‹số›·‹tên›" ở Bán/SX/Đóng gói); ✅ **NR-4 người thao tác** (cột `production_wips.operator`, `0037`, gắn `useAuth().fullName`, cột "Người ghi" ở `/wip`); ✅ **NR-6 QR in+quét** (thêm dep `qrcode`+`html5-qrcode`; in tem QR mã lô ở Nhập hàng qua `QrTemLoIn`; màn `/qr` quét camera/gõ tay tra lô — quét camera cần test máy thật). **Còn:** NR-3 (spec tái cấu trúc DM thành phẩm — chờ danh sách kiểu chế biến) · NR-1 (biểu mẫu giấy — chờ tờ mẫu). Chi tiết [`hop-2026-09-02 §8`](docs/trien-khai/hop-2026-09-02-form-nhap-trace-gia-qc.md).
- **Đợt 3 (2026-09-05):** ✅ **A1 nav cây module-centric** (`NHOM_NAV`→`CAY_NAV`, nhóm gập/mở nhớ theo máy, deep-link tra danh mục trong module `catalog?tab=…`, `CatalogScreen` đọc `useSearchParams` — MỘT nguồn danh mục; gate 2 bộ phận giữ nguyên); ✅ **A2 mobile-first** `/imports`+`/wip` (nút chính full-width lên đầu ở điện thoại; 360px+130% không cuộn ngang; vá dialog chốt còn-dở `flex-wrap`); ✅ **A3 khép vòng G1** (xem dòng G1 trên); ✅ **A4 báo cáo TP ngày** (`/wip` gom theo xưởng×người thao tác + nút "Gửi báo cáo lên hệ thống" ghi `audit_log` action `gui`, tách khỏi chốt ngày — không migration). Chi tiết [`hop-2026-09-02 §8 Đợt 3`](docs/trien-khai/hop-2026-09-02-form-nhap-trace-gia-qc.md).
