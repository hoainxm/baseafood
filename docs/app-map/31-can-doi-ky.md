...
<!-- updated: 2026-09-10 (f) — Ô CHỌN LOẠI NL (Tạo kỳ) GỘP size: optLoaiNL dedupe danh mục material_types bằng hoNguyenLieu → chỉ hiện MỘT "Bạch tuộc 2 da", không còn "… lớn (80↑)"+"… nhỏ (80↓)" riêng. Value=tên họ nên hiển thị đúng + lưu thẳng (thay cách "giữ tên đã chọn" cũ). Danh mục GỐC vẫn đủ size. -->
<!-- updated: 2026-09-10 (b) — QUY TẮC "kỳ dùng tên HỌ, không size": BalancingScreen.luuKy chuẩn hoá materialTypeName bằng hoNguyenLieu() KHI LƯU → chọn "… lớn (80↑)"/"… nhỏ (80↓)" đều lưu "Bạch tuộc 2 da". prefill chi phí tìm theo họ. Tên không có mốc 80↑/↓ giữ nguyên. Xem §"Một kỳ = một HỌ nguyên liệu". -->
...
⇒ **QUY TẮC: tên kỳ luôn là tên HỌ, không mang size.** Lớn hay nhỏ đều là "Bạch tuộc 2 da". Người dùng chọn biến thể `… lớn (80↑)` / `… nhỏ (80↓)` trong ô loại NL cũng KHÔNG sao: `BalancingScreen.luuKy` **chuẩn hoá `material_type_name` bằng `hoNguyenLieu()` khi LƯU** nên `balancing_periods.material_type_name` không bao giờ mang size — trước đây để tên có size thì kỳ mất nửa số liệu (hút lọc theo họ nhưng nhãn kỳ gây hiểu nhầm). **Ô Combobox loại NL GỘP biến thể size về HỌ** (`optLoaiNL`: dedupe danh mục material_types bằng `hoNguyenLieu`) — chỉ hiện MỘT mục `Bạch tuộc 2 da`, không hiện `… lớn (80↑)` + `… nhỏ (80↓)` riêng. Value = tên họ nên chọn xong ô hiển thị đúng + lưu thẳng (luuKy chuẩn hoá lần nữa phòng gõ tay). **Danh mục material_types GỐC (màn Danh mục, sổ nhập) vẫn giữ đủ size để truy nguyên** — chỉ ô chọn loại của Cân đối mới gộp. Prefill chi phí tìm kỳ trước theo họ. Tên không có mốc `80↑/↓` (VD `Bạch tuộc 1 da`, `Mực ống 7cm`) giữ nguyên.
