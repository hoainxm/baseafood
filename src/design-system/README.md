...
| Bảng tổng hợp cho trang Báo cáo | `BangTong` — prop **`dinhDau`** (bảng dài): khung cuộn riêng cao ≤70dvh, hàng tên cột **dính đỉnh** + dòng tổng dính đáy (utility `bang-dinh-dau` trong `tokens.css`; sticky theo trang không chạy xuyên khung cuộn ngang nên phải cuộn trong khung). Cột số căn phải `tnum`, hàng TỔNG CỘNG ở chân tự cộng theo `tong` từng cột. Component cha lo gom nhóm; `BangTong` chỉ hiển thị. Prop **`chon?: ChonBang<T>`** bật thêm cột ô tick ở đầu bảng (`{daChon: Set<khóa>, doi(k), doiTatCa(keys, bat), nhanDong?(row)}`): ô tick ở header = chọn/bỏ mọi dòng ĐANG HIỆN của bảng đó, dòng đã tick tô `bg-primary/5`. `BangTong` **không giữ state** — màn hình giữ `Set` khóa và tự cộng tổng / in / xử theo lô (khuôn phần mềm kế toán kho). Nhiều bảng chia nhóm dùng CHUNG một `ChonBang` để tick xuyên nhóm. |
...
| Các con số bối cảnh/tổng | `ThongKe` (lưới thẻ KPI: nhãn nhỏ + số TO + icon màu) — sinh động hơn nhồi một dòng. **Bỏ trống `cot` ⇒ tự suy theo số thẻ**: ≤4 một hàng · 5 thẻ một hàng ở xl (lg 3+2, thẻ hẹp icon lên trên) · 6 thẻ 3×2 · nhiều hơn 4 cột. Điện thoại: cột tối thiểu 9.5rem (chữ 130% tự về 1 cột), thẻ cuối lẻ trải hết hàng. Truyền `cot` chỉ khi cần ép. `ContextBar` chỉ dùng cho thanh dính "đang xem ngày/xưởng nào" trong màn nhập liệu. |
| Biểu đồ trong Báo cáo | `BieuDoCot` — thanh NGANG + đường trung bình nét đứt, một màu thương hiệu, thuần HTML + token (ăn theo cỡ chữ). Mặc định sắp giảm dần theo giá trị; chuỗi theo NGÀY thì bật `giuThuTu` để giữ trình tự thời gian (giá trị hiện cuối mỗi thanh, không cuộn ngang). Ưu tiên hơn `BieuDoCotDoc` (cột đứng) khi nhiều mục — cột đứng dễ tràn ngang phải cuộn. Mỗi mục có thể kèm `chiTiet` (ReactNode) → hiện thẻ TÓM TẮT khi di chuột (thẻ tự xổ xuống/lên tùy vị trí để không trồi khỏi vùng chart; rộng tới **~26rem** ở desktop, hẹp lại **16rem** ở điện thoại, nội dung **xuống dòng — không cắt chữ**, để tóm tắt đủ thông tin trong ngày mà không tràn ngang mobile); thanh có animation mọc từ 0 + hiệu ứng hover. KHÔNG thêm thư viện chart. |
