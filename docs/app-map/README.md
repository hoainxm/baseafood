...
| Tồn NGUYÊN LIỆU theo ngày/lô, lấy NL ra chế biến, cấp đông/xả đông theo lô, nối `/nxt-nl`·`/nxt-kho`·`/ton-kho-thang` về một sổ | [38-so-ton-lien-tuc](38-so-ton-lien-tuc.ba-spec.md) (ba-spec) → [34](34-btp-san-xuat-kho.ba-spec.md) · [31](31-can-doi-ky.md) · [36](36-so-kho-thang.md) |
...
| [38-so-ton-lien-tuc.ba-spec.md](38-so-ton-lien-tuc.ba-spec.md) | **ba-spec (WHAT, chưa build)**: sổ tồn NGUYÊN LIỆU liên tục = một sổ SỰ KIỆN kho theo (lô × vị trí × xưởng) — `kho tươi → sàn SX → tiêu hao / còn dở → chờ cấp đông → kho dự trữ → xả đông`; lô S (chuyến) / F (đông, n gốc) / K (kế thừa cutover); `LAY_RA`≠`TIEU_HAO`, `effective_date` + "chốt bị hở", cấm dồn âm, ánh xạ mã kế toán 2 tầng, phái sinh từ mốc cutover; thay dần 5 sổ tồn song song (`/nxt-nl`, `/nxt-kho`, `/ton-kho-thang`…) bằng "một sự thật" qua giai đoạn song song. 24 AC có Test/Assert, 6 Open chờ chủ dự án | `src/lib/inventoryMaterial.ts`, `src/lib/truyXuatLo.ts`, `src/features/reports/MaterialNxtScreen.tsx`, `src/features/reports/WarehouseNxtScreen.tsx`, `src/features/production/WipProductionScreen.tsx` |
