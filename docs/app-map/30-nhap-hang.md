...
- Bước ② có 3 lối: thẻ **xe đã chở** (một chạm) · **"Xe khác — gõ tay"** (`goXeTay` mở 2 ô, xong bấm "Xong, sang gõ kg") · **"Không ghi xe"** (`boQuaBuocXe`). Đổi đại lý khi ghi chuyến MỚI ⇒ xóa xe của đại lý cũ.
...
- **Cột NGÀY** (phải ở ≥1280px `xl`, dưới phiếu ở màn hẹp): "Đã ghi ngày ‹ngày› · xưởng ‹X›" = `nhomNgayPhien` (gom theo ĐÚNG ngày+xưởng của phiếu, **không phụ thuộc bộ lọc sổ**) + tổng ngày + nút **Sửa** từng chuyến (`moSuaChuyen`, ẩn khi ngày đã chốt) → khỏi đổi tab "Sổ ngày"; dưới đó là `KhoiPheLieuNgay`. Strip chốt ngày vẫn ở cuối màn.
...
- **Không đè số người dùng đã gõ**: `chonDaiLy`/`chonXuong` chỉ dựng lại bảng khi `!dongBang.some(dongCoData)` và KHÔNG chạy khi đang sửa chuyến đã ghi. "Lưu & thêm chuyến khác" giữ đại lý ⇒ bảng mới lại theo mẫu (`bangMoi`).
