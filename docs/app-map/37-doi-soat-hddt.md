...
<!-- updated: 2026-09-22 — v6.8 SO ĐỦ 3 CỘT (trước thuế + thuế + tổng) Ở TỪNG HÓA ĐƠN. Trước chỉ so tổng thanh toán. Nay vòng ghép trong `doiSoat` cộng thêm `pmChua`/`pmThue` (sổ đã có sẵn `DongPhanMem.chuaThue/thue`), tính `chenhChua`/`chenhThue`, và GHI ĐỦ 3 CẶP vào `bangChung` mỗi dòng (HĐ chưa·thuế·tổng | sổ chưa·thuế·tổng | chênh từng cột) — kế toán soi tận nơi, KHÔNG thêm cột (chốt với chủ dự án: bản gọn). Nhãn cột A (KẾT QUẢ): (1) TỔNG lệch → "LỆCH TIỀN" (như cũ); (2) tổng KHỚP nhưng THUẾ lệch → nhãn MỚI "LỆCH THUẾ" (`tong.lechThue`, trangThai vẫn "LECH" để gộp nhóm L của bảng ĐỐI CHIẾU ⇒ 9/9 tự kiểm + đẳng thức A=K+L+T giữ nguyên); (3) tổng+thuế khớp, chỉ TRƯỚC THUẾ lệch → vẫn "KHỚP" + ghi chú "thường do sổ ghi trước thuế đã trừ chiết khấu/cộng phí" (KHÔNG báo oan — vd K26TUE-254474 chênh trước thuế 91.666 = chiết khấu, tổng+thuế khớp). Bảng TRẢ LỜI (`doiSoatXuatTongHop.ts`) tách 2 dòng LỆCH TIỀN / LỆCH THUẾ lọc theo `ketLuan`; dòng nhóm L đổi nhãn "LỆCH (tiền + thuế)". KIỂM THẬT file "…22_9.xlsx": khớp 2049 · lệch tiền 8 · LỆCH THUẾ 0 · chưa kê 1494 · tự kiểm 9/9. build+lint xanh. -->
<!-- updated: 2026-09-22 — v6.7 BỎ DÒNG GHI CHÚ TAY của kế toán cuối sheet HĐĐT (`mstHopLe` + gate trong `parseSheetHoaDon`). Kế toán để KHỐI GHI CHÚ dưới bảng ("CHÊNH LỆCH THUẾ VAT", "HOÁ ĐƠN CTY CHƯA KÊ KHAI", "SỐ CỦA LINH 6 THÁNG", "HOÁ ĐƠN CTY KHÔNG SỬ DỤNG" = 3.653.660…) với con số rơi vào ô Tên/MST/Chưa thuế → engine cũ nhận nhầm là HÓA ĐƠN: gắn "CHƯA CÓ TRONG PMEM" 6 dòng + gắn 3.653.660 là "LỆCH THẬT phải sửa" (số kế toán đang TỰ TÍNH, báo oan). CHỮA: gate cũ `!soHoaDon && !kyHieu` → `!kyHieu && !mstHopLe(MST bán)` — hóa đơn thật LUÔN có ký hiệu HOẶC MST bán 10/13 chữ số; dòng ghi chú không có cả hai nên bỏ qua (không cộng, không nhãn, không đụng ô của kế toán). KIỂM THẬT file gốc: 0 dòng "lệch thật", số 3.653.660 biến khỏi kết quả, "chưa kê" 1500→1494, tổng trước thuế 299.173.468.893 (khớp số kế toán 299.173.377.225, chênh 91.668đ = 1 dòng chiết khấu). CHỈ áp cho sheet HĐĐT/MTT (`parseSheetHoaDon`), KHÔNG đụng `parseSheetPhanMem`. -->
