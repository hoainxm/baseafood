...
- ✅ **A3 — Khép vòng G1 "còn dở"→Cân đối:** trả lại ô "còn dở" khi chốt ngày SX **tách theo loại NL** (`production_locks.leftover_by_material`, mig `0038`); `/nxt-nl` cộng vào đông gửi (cột riêng "Còn dở SX"), **tồn cuối = tồn đầu + đông gửi + còn dở SX − xả đông** (đối chiếu tay: có/không leftover đều khớp). ⚠️ conDoSX giữ cột RIÊNG vì quy tắc "chia NL cho từng bảng" **chưa chốt** (tránh cộng đôi ngầm với carryOver kế toán). Doc: [`31-can-doi-ky`](../app-map/31-can-doi-ky.md).
