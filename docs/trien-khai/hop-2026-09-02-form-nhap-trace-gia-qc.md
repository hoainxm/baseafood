...
### Đợt 3 — 4 chức năng "build được ngay" (cập nhật 2026-09-05)
Chủ dự án chốt (hỏi-đáp): **A1 = tái cấu trúc IA theo module (cây sâu)** · **A3 = capture còn dở theo họ NL + gộp vào tồn (đầy đủ)**.
- ✅ **A1 — Nav cây module-centric + danh mục trong module (QĐ-9):** `NHOM_NAV`→`CAY_NAV` (AppShell), nhóm gập/mở (nhớ theo máy), mỗi module chứa màn + **deep-link tra danh mục** `catalog?tab=…`; `CatalogScreen` đọc tab bằng `useSearchParams` — **MỘT nguồn danh mục** (không nhân bản). Gate 2 bộ phận giữ nguyên (deep-link kiểm quyền theo `catalog`). Doc: [`02-pages-navigation`](../app-map/02-pages-navigation.md), [`design-system/README`](../../src/design-system/README.md).
- ✅ **A2 — App nhập mobile-first (#19/#27):** `/imports` + `/wip` nút CHÍNH full-width + lên đầu trên điện thoại; đối chiếu 360px + 130% cỡ chữ không cuộn ngang; vá dialog Chốt ngày (hàng còn-dở `flex-wrap`) khỏi đội bề rộng ở 375px.
- ✅ **A3 — Khép vòng G1 "còn dở"→Cân đối (HƯỚNG 1, chốt 2026-09-06):** ô "còn dở" khi chốt ngày SX **tách theo loại NL** (`production_locks.leftover_by_material`, mig `0038`) là **NGUỒN CHÍNH của đông gửi**: `dongGui = còn dở SX nếu có, chưa có mới lấy Chuyển kỳ âm` — **KHÔNG cộng cả hai** (hết cộng đôi). Kế toán khỏi khai Chuyển kỳ âm; xả đông (Chuyển kỳ dương) vẫn khai. **Tồn cuối = tồn đầu + đông gửi(hoà giải) − xả đông**; dữ liệu cũ không hồi quy (đối chiếu tay 4 case). `/nxt-nl` gộp 1 cột "Đông gửi" (nhãn "(SX)"). Doc: [`31-can-doi-ky`](../app-map/31-can-doi-ky.md).
- ✅ **A4 — Báo cáo TP ngày gom + gửi tách (mở rộng NR-4):** `/wip` thêm section gom theo (xưởng × người thao tác) + nút **"Gửi báo cáo lên hệ thống"** (ghi `audit_log` action `gui`/`production_report`, **không migration**) — tách khỏi "Chốt ngày"; mốc "đã gửi" nhớ theo máy để phản hồi. Doc: [`34-btp`](../app-map/34-btp-san-xuat-kho.ba-spec.md).
- ⏭️ **Còn CHỜ (không đụng):** giá bình quân gia quyền (QĐ-7 chưa chốt PA) · điểm sinh mã lô · NR-3 tái cấu trúc DM · OCR/ảnh QC (chưa có Storage) · siết RLS server.
