...
covers: src/features/production/WipProductionScreen.tsx, src/features/warehouse/ReserveWarehouseScreen.tsx, src/features/orders/SalesOrderScreen.tsx, src/features/packaging/PackagingScreen.tsx, src/features/catalog/CatalogScreen.tsx, src/lib/inventory.ts, supabase/migrations/0011_wip_san_xuat_kho_don.sql, supabase/migrations/0026_dong_goi_thanh_pham.sql, supabase/migrations/0030_production_customer_components.sql, supabase/migrations/0031_product_split_blockspec.sql, src/features/production/wipHelpers.ts, src/features/production/BangDongSX.tsx
last_verified: 2026-09-21
<!-- re-verified: 2026-09-21 — tách file WipProductionScreen (P2-7): luật "tách râu + bao tử cùng giá ⇒ tổng = râu + bao tử" và "dòng đủ lưu = có mặt hàng + tổng>0" nay ở `wipHelpers.ts` (laTach/tongDong/dongDayDu) kèm vitest — khớp mô tả trong doc, không đổi. -->

<!-- updated: 2026-09-18 — ĐÃ BUILD trace mẻ ↔ lô (QĐ-6): /wip dòng sản lượng có nút "Gắn lô NL" (quét tem / gõ mã / chọn lô NL cùng xưởng trong 45 ngày, kg TÙY CHỌN) + "Tem" (tem QR lô BTP, nhãn suy ra `B‹xưởng›-yymmdd-‹4 ký tự cuối id›`); /packaging tương tự "Gắn lô BTP" (ưu tiên lô cùng mặt hàng, cửa sổ 120 ngày vì hàng đông trữ lâu) + tem lô TP `T…`. Ghi vào bảng `lot_inputs` (mig 0046); KHÔNG thêm cột vào production_wips/packagings. Tùy chọn, không chặn ghi sản lượng. Xóa mẻ/phiếu KHÔNG xóa dây chuyền lô (hộ chiếu hiện "bản ghi không còn" + nhãn chụp lúc gắn). Thiết kế: ../spec/qr-truy-xuat-lo.md. -->
<!-- updated: 2026-09-10 — (nhập liệu nhanh) BangDongSX (bảng ghi thành phẩm ở /wip) BỌC `[data-luoi-phim]`, các ô số khai `navCol` (sl·block·quycach·rau·baotu) → ↑/↓/Enter nhảy dọc theo cột kiểu Excel (Tab đi ngang như cũ). Mọi ô số cũng hiểu BIỂU THỨC "1+2"→3. Cơ chế ở design-system (NumberField `navCol` + parseSoHoacBieuThuc) — xem README §5d; màn chỉ khai `navCol`, KHÔNG viết logic. KHÔNG đụng dữ liệu/logic sản xuất. -->
<!-- updated: 2026-09-07 — (QĐ-1/NR-1 biểu mẫu giấy, họp [2026-09-02](../trien-khai/hop-2026-09-02-form-nhap-trace-gia-qc.md)) /wip THÊM nút "In phiếu trống" ở chế độ "Sổ ngày & báo cáo" (state inPhieuTrong) → mở `PhieuTrongTPNgay` (src/features/shared/PhieuTrongIn.tsx): biểu mẫu A4 TRỐNG "Báo cáo bán thành phẩm hàng ngày" cho xưởng ghi tay (cột STT·Tên BTP/Quy cách·Số lượng·Đơn giá·Thành tiền·Ghi chú + dòng NGÀY trống + Tổng cộng, primitive PhieuIn) — khớp tab "Bán Thành Phẩm Hàng Ngày" của file Excel mẫu. KHÔNG đụng dữ liệu/logic. Verify preview render đúng 6 cột. -->
<!-- updated: 2026-09-05 — (A3 khép vòng G1) TRẢ LẠI ô "còn dở nguyên liệu" khi chốt ngày SX (đã bỏ ở bản giản lược 2026-08-25) NHƯNG dạng TÁCH THEO LOẠI NL: dialog Chốt ngày có danh sách (Combobox loại NL từ danh mục, tạo tại chỗ + NumberField kg), lưu `production_locks.leftover_by_material` (mig 0038) + `leftover_kg`=tổng. Sổ /nxt-nl cộng vào đông gửi (conDoSX). Xem 31-can-doi-ky §Tồn kho NL. LƯU Ý: đây KHÁC ô "còn dở" bản đầu (1 số/ngày) — nay tách loại để ráp được vào tồn theo họ NL. -->
<!-- updated: 2026-09-05 — (A4 mở rộng NR-4) màn /wip THÊM section "Báo cáo công việc ngày": gom dòng đang xem theo (xưởng × người thao tác) + nút "Gửi báo cáo lên hệ thống" — TÁCH khỏi "Chốt ngày" (khoá sửa). Gửi = ghiNhatKy(audit_log) action="gui" entity="production_report" (KHÔNG migration); mốc "đã gửi" nhớ theo máy (localStorage bsf.wip-report-sent.v1) chỉ để phản hồi tại chỗ. -->
<!-- updated: 2026-09-05 — (A2 mobile-first) nút CHÍNH "Ghi thành phẩm" (/wip) + "Ghi nhập trong ngày" (/imports) full-width + lên ĐẦU trên điện thoại (< sm); hành động phụ xuống dưới. Dialog Chốt ngày: hàng còn-dở dùng flex-wrap + min-w-0 để không đội bề rộng lưới dialog ở 360px. -->

<!-- updated: 2026-09-06 — FORM-FIRST (NR-2): /wip bỏ "trang quản lý + Dialog ghi", thay bằng toggle 2 chế độ đầu màn: "📝 Ghi nhập" (form ghi thành phẩm render INLINE — đầu phiên + BangDongSX — tự mở phiếu trống qua useEffect, KHÔNG modal) ↔ "📖 Sổ ngày & báo cáo" (ThongKe + bộ lọc + RecordTable sổ). Chốt ngày (còn-dở) + Báo cáo công việc ngày (A4, gửi) là khối DÙNG CHUNG dưới cùng (bám bộ lọc). Dùng lại 100% logic (moThem/luuPhien/xongPhien/chốt/còn-dở/A4) — chỉ re-parent form (biến `formGhi`). 2 useEffect: mở phiếu trống khi vào "Ghi nhập"; đồng bộ ngày+xưởng phiếu → bộ lọc. Lưu xong → datLaiPhien(null) → effect mở phiếu mới. Bỏ `dongKhongLuu`. Đối xứng /imports (xem 30-nhap-hang). -->
<!-- prev last_verified: 2026-09-05 -->
ttl_days: 90
<!-- updated: 2026-08-27 — ĐẢO TRỤC GOM màn ghi thành phẩm /wip theo SỔ GIẤY THẬT ("Báo cáo bán thành phẩm hằng ngày" xưởng Đông). Trước gom theo LOÀI (Product.category); NAY gom theo NHÓM = (KIỂU CHẾ BIẾN × KHÁCH HÀNG) — đúng bố cục sổ "2 da chần · Peacock" → nhiều dòng thành phẩm (bộ phận+size) + kg. Thay đổi: (a) DongSX bỏ trục `category`, thêm `processingType` + `customerName` làm khóa nhóm `khoaNhom()`; (b) "Thêm nhóm" = 2 combobox (Kiểu chế biến [gợi ý từ Product.processingType, tạo tại chỗ] × Khách hàng) thay cho "Thêm loài"; khách chuyển từ cột-mỗi-dòng LÊN cấp nhóm; (c) dòng chọn mặt hàng từ TOÀN BỘ danh mục (gõ tìm), tạo mới tại chỗ gắn processingType=nhóm (category mặc định "Bạch tuộc" — xưởng Đông); (d) migration 0033 thêm cột `production_wips.processing_type` (snapshot chế biến trên dòng WIP, không phụ thuộc mặt hàng đã gắn processingType chưa) — WipProductionItem.processingType (types.ts), map ở repo.ts; (e) sổ xem thêm cột "Chế biến". GIỮ nguyên: tách râu/bao tử (component 0030), block×quy cách, chốt/ghi bù, đơn vị kg. Verify: ghi "2 da chần · Peacock" → "Râu 700" 2.510 kg → production_wips lưu processing_type/customer_name đúng. LƯU Ý: đây là điều chỉnh quyết định "gom theo LOÀI" của bản 2026-08-26 sau khi soi tờ sổ thật; mục tiêu cutover 1/9 = ghi nhận trung thực lượng làm ra ngày. -->
...
<!-- updated: 2026-08-26 — TINH CHỈNH màn ghi thành phẩm /wip (chốt với chủ dự án 2026-08-25/26) sau khi thử tay. (1) Entry NHÓM THEO LOÀI (Product.category — Bạch tuộc/Mực/Cá…): mỗi loài một cụm (card: header tên loài + bảng trải ngang + nút "Thêm thành phẩm"), Combobox "Thêm loài" ở cuối (options = CATEGORIES chưa dùng) để mở cụm mới; Combobox thành phẩm trong cụm CHỈ lọc mã cùng category (loài). GOM THEO LOÀI chứ KHÔNG theo loại NL vì: loài có sẵn trên cả 141 mã seed (materialTypeId TRỐNG trên seed) và gộp lớn/nhỏ tự nhiên ("Bạch tuộc 2 da lớn/nhỏ" đều loài Bạch tuộc). Mã tạo mới tại chỗ (themMatHang) gắn luôn category=loài đang mở. products.materialTypeId GIỮ NGUYÊN (dùng ở nơi khác) nhưng KHÔNG phải cấp gom màn này. (2) TÁCH râu/bao tử — mũi tên chevron hiện ở MỌI dòng (chốt 2026-08-26 sau khi chủ dự án tưởng mất tính năng): bấm để mở/đóng 2 ô râu/bao tử; tách "tính" (quantity_kg = tổng, ghi component_rau/bao_tu_kg) KHI đã nhập râu/bao tử — dùng cột 0030 nên KHÔNG phụ thuộc 0031. Cờ **products.split_components** (migration 0031, ChoiceGroup "Có tách/Không tách" ở Danh mục) giờ chỉ để TỰ MỞ SẴN ô tách khi chọn mã đó (laCoTach() trong types.ts), không còn là điều kiện hiện mũi tên. Dialog Sửa cũng luôn có khối râu/bao tử. (3) QUY CÁCH BLOCK — nhập NGAY trên dòng ghi (chốt 2026-08-26): cột "Số block × quy cách" có 2 ô (số block, kg/khối); quantity_kg tự = block × kg/khối khi mã không tách. Ô kg/khối TỰ ĐIỀN từ **products.block_spec_kg** (migration 0031) nếu mã đã có, và khi Lưu phiên thì LƯU NGƯỢC kg/khối vừa nhập về mặt hàng (đúng ý "gắn trên mặt hàng" — lần sau tự điền; setMatHang trong luuPhien). Vẫn set trực tiếp được ở Danh mục. DongSX mang blockSpecKg (chỉ để tính + nhớ, KHÔNG có cột trên production_wips). Field/NumberField thêm prop anNhan (ẩn nhãn, giữ aria-label) để nhồi vào ô bảng. GỘP 1 danh sách LOÀI chuẩn: types.ts CATEGORIES thêm "Bào ngư" (trước thiếu), BỎ NHOM_TP ở CatalogScreen — cả tab Mặt hàng lẫn Loại NL dùng chung CATEGORIES. -->
