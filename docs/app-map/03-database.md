...
ttl_days: 90
<!-- updated: 2026-09-15 — thêm 0045 (`monthly_stock_ledger.storage_location text not null default ''`): VỊ TRÍ hàng đang nằm (TÊN kho — kho xí nghiệp hoặc kho thuê ngoài trong `storage_locations`), rỗng = ở chính kho của sổ (`warehouse`). CHỈ-THÊM cột, idempotent, có ROLLBACK. ⚠ PHẢI chạy trên DB thật TRƯỚC khi deploy bản app có cột Vị trí — toRow gửi `storage_location` luôn, thiếu cột thì mọi lần ghi sổ tháng lỗi. Cột `origin` giữ tên cũ nhưng NGHĨA nay là số INVOICE (dữ liệu nạp từ cột 4 bảng kê thực chất là số invoice, VD SBT2026004). Migration cao nhất nay = 0045. -->
<!-- updated: 2026-09-15 — thêm 0044 (kho LƯU trữ). (a) Bảng MỚI `storage_locations` (id, site_id, code, name, kind 'noi-bo'|'thue-ngoai', address, phone, note, timestamps) — danh mục kho nhà + kho lạnh THUÊ NGOÀI, seed 3 dòng kho-baseafood/kho-hong-phu/kho-anh-duong bằng `on conflict do nothing` (id trùng seed app ở catalogRepo.seedStorageLocations nên chạy 2 chế độ không nhân đôi). (b) CHỈ-THÊM cột `nxt_snapshots.storage_location text not null default ''` + index (site_id, storage_location) — LƯU TÊN kho (không phải id, khớp cách `monthly_stock_ledger.warehouse` làm); rỗng = chưa gán ⇒ app hiểu là kho nhà, dữ liệu trước 0044 không bị ép sửa. Idempotent, trigger cap_nhat_thoi_diem_sua, RLS anon+authenticated (như 0041/0043), có ROLLBACK. ⚠ CHƯA chạy trên DB thật lúc viết doc — ghi nxt_snapshots sẽ lỗi cột thiếu cho tới khi chạy 0044. Xem 04-tang-du-lieu (BANG_STORAGE_LOCATION) + 32-danh-muc (tab Kho lưu trữ) + màn /nxt-kho. Migration cao nhất nay = 0044. -->
<!-- updated: 2026-09-14 — thêm 0043 (reconciliation_runs): LƯU bản đối soát hóa đơn /doi-soat theo TÀI KHOẢN. Cột: id, site_id, user_id, owner_username, owner_name, title, period, status ('draft'|'official'), threshold numeric, file_name, file_b64 (file Excel gốc base64 — để mở lại chạy lại), options jsonb, summary jsonb (ảnh chụp tóm tắt, đóng băng bản chính thức), created_at/updated_at. CHỈ-THÊM bảng, idempotent, trigger cap_nhat_thoi_diem_sua, RLS permissive như 0041 (khối comment sẵn câu siết user_id=auth.uid() cho nhánh 0021) + ROLLBACK. Base64-trong-bảng thay vì Storage → nằm dưới RLS bảng, tránh cấu hình ACL Storage theo user. Xem 04-tang-du-lieu (BANG_RECONCILIATION_RUN) + 37-doi-soat-hddt. Migration cao nhất nay = 0043. -->
<!-- updated: 2026-09-10 — thêm 0042 (import_shipments.operator text, nullable): NGƯỜI GHI chuyến nhập (họ tên/username tài khoản lúc lưu) — song song với production_wips.operator (0037). App gắn useAuth().fullName khi lưu chuyến (MaterialImportScreen.luuPhien), hiện "Người ghi" ở sổ nhập; repo.ts BANG_IMPORT_SHIPMENT map operator (+ ImportShipment.operator, vaDongCu để rỗng). Migration CÓ backfill: chuyến cũ operator rỗng → full_name của nnttruc (rỗng thì 'nnttruc'). Chỉ-thêm-cột, idempotent, có ROLLBACK; import_shipments đã trong RLS 0021. Migration cao nhất nay = 0042. Kèm sửa dữ liệu MỘT LẦN (không phải migration) ở docs/ops/fix-2026-09-10-swap-31-08-01-09-va-operator-nnttruc.sql: swap ngày nhập 31/08↔01/09 (chuyến+dòng+phế liệu+chốt, cả 3 xưởng) + ghi lại audit_log admin→nnttruc (chạy SQL Editor, owner update được audit_log dù 0025 revoke của authenticated). -->
<!-- re-verified: 2026-09-09 15:00 — 0041_monthly_stock_ledger.sql: create table if not exists, cột tiếng Anh snake_case, kg numeric(16,3)/ctn numeric(14,3)/giá numeric(18,2), nullable kg_per_ctn/unit_price, RLS anon+authenticated, có ROLLBACK — khớp code + quy ước. -->
<!-- updated: 2026-09-09 — thêm 0041 (monthly_stock_ledger: SỔ KHO THEO THÁNG dương lịch, hạt Tháng × Nhóm × Mặt hàng × Size × Xuất xứ). Cột: period 'YYYY-MM', category, warehouse, item_name, size, origin, import_date, kg_per_ctn, unit_price, open/in/out (mỗi cái _ctn + _kg), carried_from_id, sort_order, note. Tồn cuối (kiện & kg) + tiền còn lại SUY ở app không lưu; tồn ĐẦU lưu (snapshot kế thừa). Dồn kỳ = id tất định `carry|<id-nguồn>` (chạy lại chỉ cập nhật, không nhân đôi). CHỈ-THÊM bảng, idempotent, RLS anon+authenticated (chưa vào vòng siết 0021). Xem 04-tang-du-lieu (BANG_MONTHLY_STOCK) + 36-so-kho-thang + màn /ton-kho-thang. Migration cao nhất nay = 0041. -->
...
| `0034_backfill_processing_type_products.sql` | 🟡 | **Backfill kiểu chế biến + bổ sung mặt hàng thiếu.** Chủ yếu DỮ LIỆU (có `add column if not exists processing_type` tự-guard để chạy được cả khi DB chưa chạy `0027`). ① `UPDATE products SET processing_type` suy TỪ TÊN cho dòng còn trống (CASE mirror `lib/catalogRepo.suyKieuCheBien`, khớp trước thắng, không đè nhãn đã sửa tay). ② `INSERT` 8 mặt hàng thật (bạch tuộc 2 da, mức gộp) thiếu hẳn ở 141 mã — `finished_good_code=''` (chưa ánh xạ), id `'mh-bs-*'` tất định + `ON CONFLICT DO NOTHING`. Idempotent, có `ROLLBACK`. Đối xứng seed local `seedProducts`. KHÔNG đụng `thanh_pham` (141 mã kế toán). Xem [32-danh-muc.md](32-danh-muc.md). |
