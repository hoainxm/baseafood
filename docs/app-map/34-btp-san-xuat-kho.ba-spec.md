...
<!-- updated: 2026-08-26 — TINH CHỈNH màn ghi thành phẩm /wip (chốt với chủ dự án 2026-08-25) sau khi thử tay. (1) Entry NHÓM THEO LOẠI NGUYÊN LIỆU: mỗi loại NL một cụm (card: header tên NL + bảng trải ngang + nút "Thêm thành phẩm"), Combobox "Thêm loại nguyên liệu" ở cuối để mở cụm mới; Combobox thành phẩm trong cụm CHỈ lọc mã cùng material_type_id (chọn NL trước → thấy đúng thành phẩm của nó; tạo mới tại chỗ tự gán NL). (2) TÁCH râu/bao tử KHÔNG bật cho mọi mã: gắn cờ **products.split_components** (migration 0031) ở Danh mục (CatalogScreen ChoiceGroup "Có tách/Không tách"); chỉ mã bật cờ mới hiện mũi tên chevron đầu dòng để mở 2 ô râu/bao tử (BỎ nút "Tách" cũ; laCoTach() trong types.ts). Dialog Sửa cũng ẩn khối râu/bao tử khi mã không có cờ. (3) QUY CÁCH BLOCK gắn trên mặt hàng: **products.block_spec_kg** (kg/khối, migration 0031) ở Danh mục (NumberField); màn ghi tự tính quantity_kg = số block × quy cách khi mã không tách + hiện hint "× N kg/khối". Field/NumberField thêm prop anNhan (ẩn nhãn, giữ aria-label) để nhồi vào ô bảng. -->
